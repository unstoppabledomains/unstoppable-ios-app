...
extension PrivateKeyStorage {
    ...
    func retrieveValue(for key: String, isCritical: Bool = false) -> String? {
        do {
            ...
            Debugger.printFailure("Failed to get the priv key for the key: \(key) with error \(error.localizedDescription)", critical: isCritical, suppressOnlineLogging: !isCritical)
            return nil
        }
    }
    ...
    func retrievePrivateKey(for pubKeyHex: String, isCritical: Bool = false) -> String? {
        ...
        do {
            ...
            Debugger.printFailure("Failed to get the priv key for the key: \(pubKeyHex) with error \(error.localizedDescription)", critical: isCritical, suppressOnlineLogging: !isCritical)
            return nil
        }
    }
