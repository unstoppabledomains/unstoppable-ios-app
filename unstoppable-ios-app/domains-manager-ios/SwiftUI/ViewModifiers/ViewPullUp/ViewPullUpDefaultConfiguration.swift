...
extension ViewPullUpDefaultConfiguration {
    ...
    static func recordDoesNotMatchOwner(chain: BlockchainType,
        ...
        return .init(icon: .init(icon: icon ?? .appleIcon,
                                 size: .large),
              title: .text("\(chain.fullName) doesn’t match owners address"),
              subtitle: .label(.highlightedText(.init(text: "Owner address: \(ownerAddress.walletAddressTruncated)",
                     ...
                     actionButton: .main(content: .init(title: String.Constants.gotIt.localized(),
                                                        analyticsName: .gotIt,
                                                        action: nil)),
                     extraButton: .primaryGhost(content: .init(title: "Update records",
