...
extension UIImage {
    ...
    func getColors(quality: UIImageColorsQuality = .high) async -> UIImageColors? {
        await withSafeCheckedContinuation({ completion in
            getColors(quality: quality) { image in
                completion(image)
            }
        })
    }
    ...
    func getColors(quality: UIImageColorsQuality = .high) -> UIImageColors? {
        ...
        guard let cgImage = self.resizeForUIImageColors(newSize: scaleDownSize).cgImage,
        ...
        guard let data = CFDataGetBytePtr(dataProvider.data) else {
            return nil
        }
